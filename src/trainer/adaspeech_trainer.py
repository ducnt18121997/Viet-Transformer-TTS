...
from models.tts.adaspeech import AdaSpeech, AdaSpeechLoss
...
class AdaSpeechTrainer(BaseTrainer):

    def __init__(self, args: ArgumentParser, conf: dict) -> None:
        super().__init__(args, conf)

    def __init_dataset__(self):
        self.self_learning = self.conf["models"]["adaspeech"]["variance"]["learn_alignment"]
        ...
        self.train_loader = DataLoader(
            ...
            collate_fn=Fastspeech2Collate(            
                ...
                use_accent      = self.conf["models"]["adaspeech"]["use_cvae"],
    ...
    def __init_model__(self):
        ...
        self.model_conf = {
            ...
            "conf": {
                ...
                "hparams": self.conf["models"]["adaspeech"],
            },
        }

        self.model = AdaSpeech(
    ...
    def __init_loss__(self):
        ...
        self.loss_conf = self.conf["train"]["adaspeech"]["loss"]
        self.loss_conf.update(dict(
            pitch_feature_level  = self.conf["models"]["adaspeech"]["variance"]["variance_embedding"]["pitch_feature"], 
            energy_feature_level = self.conf["models"]["adaspeech"]["variance"]["variance_embedding"]["energy_feature"],
        ))

        self.criterion = AdaSpeechLoss(self.args.max_iter, self.loss_conf)

    def __init_optimizer__(self):
        # 1. Initialize optimizer
        self.optimizer = optim.AdamW(
            self.model.parameters(), **self.conf["train"]["adaspeech"]["optimizer"]
        )
