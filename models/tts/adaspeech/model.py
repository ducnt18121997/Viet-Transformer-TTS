...
from models.tts.adaspeech.layers import VarianceAdaptor
from models.tts.fastspeech2.layers import AccentEncoderNetwork, Postnet
...
class AdaSpeech(nn.Module):
    ...
    This is a module of FastSpeech2 described in `AdaSpeech: Adaptive 
    Text to Speech for Custom Voice`_. 

    .. _`AdaSpeech: Adaptive Text to Speech for Custom Voice`:
    ...
    def __init__(self,
        ...
        if self.building_block == "transformer":
            from models.tts.adaspeech.blocks.transformer import Encoder, Decoder
        else:
        ...
        self.encoder = Encoder(
            ...
            n_symbols   = self.idim,
            speaker_dim = self.hparams["encoder_hidden"],
        ...
        self.cln = CondionalLayerNorm(
            indims  = self.hparams["decoder_hidden"],
            outdims = self.hparams["decoder_hidden"]
        )
    ...
    def forward(
        ...
        **kwangs
        ...
        hs, text_embs    = self.encoder(text, text_masks, sid_embs)
        ...
        # decoder forward
        hs, feats_masks = self.decoder(hs, feats_masks, sid_embs)
        hs = self.cln(hs, sid_embs)
    ...
    def inference(
        ...
        **kwangs
        ...
        hs = self.cln(hs, sid_embs)
        outs = self.feats_linear(hs)
        if self.postnet is not None: outs = self.postnet(outs) + outs
