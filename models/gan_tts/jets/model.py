...
from models.gan_tts.jets.alignments import AlignmentModule, GaussianUpsampling, average_by_duration, viterbi_decode


class JETS(nn.Module):
    """JETS ESPNET module (FastSpeech2 (unsupervised) + HifiGan generator).
    ...
    def __init__(
        ...
        use_gaussian_upsample: bool = True,
        # energy predictor
        ...
        self.use_gaussian_upsample = use_gaussian_upsample
        if self.use_gaussian_upsample is True:
    ...
    def inference(
        ...
        **kwangs
        ...
        x_masks = self._source_mask(text_lengths)
        hs, _   = self.encoder(text, x_masks)  # (B, T_text, adim)
        # NOTE(by deanng): this for api
        if isinstance(sids, torch.Tensor) and sids.size(-1) == self.spk_embed_dim:
            spembs = sids.repeat(text.size(0), 1)
            sids   = None
        ...
        wav_lengths = torch.sum(d_outs, dim=1) * self.generator.upsample_factor

        return wav.squeeze(1), wav_lengths
